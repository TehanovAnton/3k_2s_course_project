...
parksRouter.put('/parks/:id', async (req, res) => {
  ...
  req.on('data', (chunk) => { body += chunk.toString(); });
  req.on('end', async () => {
    const updates = Service.updateAttributes(JSON.parse(body));
    const result = await Park.update(updates, { where: { id: params.id }, returning: true, plain: true })
      .catch((error) => { res.json(error); });
...
parksRouter.delete('/parks/:id', async (req, res) => {
  const { params } = req;
  const park = await Service.delete(res, Park, params.id);
